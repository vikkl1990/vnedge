...
export function RotateKeysModal({ exchange, label, onCancel, onSave }: Props) {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      const focusable = Array.from(formRef.current?.querySelectorAll<HTMLElement>('button:not([disabled]), input:not([disabled]), select:not([disabled]), [tabindex]:not([tabindex="-1"])') ?? []);
  ...
  return (
    <div data-modal-lock="true" className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4" role="dialog" aria-modal="true" aria-labelledby="rotate-title">
      <form ref={formRef} onSubmit={submit} autoComplete="off" className="w-full max-w-lg rounded-xl border border-line2 bg-panel p-5 shadow-2xl">
        ...
        <div className="mt-5 flex justify-end gap-3">
          <button type="button" disabled={saving} onClick={onCancel} className="rounded-md border border-line px-4 py-2 text-[11px] text-dim disabled:opacity-40">Cancel</button>
