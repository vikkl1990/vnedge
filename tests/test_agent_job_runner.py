...
def test_quantos_seed_requests_exclude_removed_scanners():
    strategy_ids = {str(request["strategy_id"]) for request in DEFAULT_SEED_REQUESTS}

    assert all("scanner" not in strategy_id for strategy_id in strategy_ids)
    assert all("scalper" not in strategy_id for strategy_id in strategy_ids)
