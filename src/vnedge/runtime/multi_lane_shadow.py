...
def _governor_paper_roster_filter(
    ...
    if not allowed:
        if (
            str(payload.get("report_id") or "") == "paper_only_survivor_registry_v1"
            and bool(payload.get("source_complete"))
        ):
            fallback = [
                spec
                for spec in specs
                if spec.mode is RunnerMode.PAPER
                and spec.lane_id in set(GOVERNED_PAPER_LANE_IDS.values())
            ]
            logger.warning(
                "completed paper-only survivor registry proposes no paper_lanes; "
                "keeping only %d governed baseline paper lane(s)",
                len(fallback),
            )
            return _ensure_primary(fallback)
        logger.warning(
